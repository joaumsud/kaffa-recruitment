...
class Tasks extends Component {
    ...
    async loadTasks() {
        let response = await fetch(`http://localhost:3001/tarefas`);
        ...
        return (
            <Row>
                ...
                <Col xs={{ span: 8, offset: 2 }} className="tasks_list">
                    ...
                    <List loadTasks={this.loadTasks} tasks={this.state.tasks.filter((task) => task.done === true)} />
                    <Button variant="red" className="float-right remove_tasks_btn">Remover todas as Tarefas</Button>
                </Col>
            </Row>
        );
    }
